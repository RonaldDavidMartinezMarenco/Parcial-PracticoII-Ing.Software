...
6. Consultar `habits_logs`: contar registros por `log_date` agrupando por
   `meal_type` para el mismo rango y `user_id`.
7. Consultar `goal_progress`: obtener `progress` por `log_date` para metas
   activas del usuario.
...
Stack obligatorio: FastAPI, SQLAlchemy, PyJWT, MySQL.